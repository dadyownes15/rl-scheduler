...
# each job has 8 features, plus a per-user current average feature.
JOB_FEATURES = 9
DEBUG = False
...
class HPCEnvFair(gym.Env):
    def __init__(self, shuffle=False, backfil=False, skip=False, job_score_type=0, user_score_type=0, batch_job_slice=0,
        ...
        print("*\t backfil:", backfil )
        print("*\t skip:", skip )
        print("*\t job_score_type:", job_score_type, job_score_type_strs[job_score_type])
        print("*\t user_score_type:", user_score_type, user_score_type_strs[user_score_type])
        print("*\t batch_job_slice:", batch_job_slice)
        print("*\t build_sjf:", build_sjf)
        print("=====================================================")
        ...
        self.user_score_type = user_score_type
    ...
    def reset(self):
        ...
        self.scheduled_scores.append(self.score_acorss_users(self.per_user_scores(self.schedule_curr_sequence_reset(self.f1_score)).values()))
        #self.scheduled_scores.append(self.score_acorss_users(self.per_user_scores(self.schedule_curr_sequence_reset(self.smallest_score)).values()))
        #self.scheduled_scores.append(self.score_acorss_users(self.per_user_scores(self.schedule_curr_sequence_reset(self.fcfs_score)).values()))
        #self.scheduled_scores.append(self.score_acorss_users(self.per_user_scores(self.schedule_curr_sequence_reset(self.f2_score)).values()))
        #self.scheduled_scores.append(self.score_acorss_users(self.per_user_scores(self.schedule_curr_sequence_reset(self.f3_score)).values()))
        #self.scheduled_scores.append(self.score_acorss_users(self.per_user_scores(self.schedule_curr_sequence_reset(self.f4_score)).values()))
            ...
            for _j in job_queue_iter_copy:
                if (self.current_timestamp + _j.request_time) < earliest_start_time:
                    if self.cluster.can_allocated(_j):
                        ...
                        scheduled_logs[_j] = score  # scheduled_logs uses job as key
    ...
    def schedule_curr_sequence_reset(self, score_fn):
        ...
        while True:
            ...
            scheduled_logs[job_for_scheduling] = score # scheduled_logs uses job as the key
    ...
    def build_observation(self):
        ...
        for i in range(0, MAX_QUEUE_SIZE):
            if i < len(self.visible_jobs) and i < (MAX_QUEUE_SIZE):
                ...
                # add user-level information to the RL agent
                # we take 1 - j's score/max(all jobs' scores) as the normalized value.
                # if it approaches to 1, means j's score is minimizal already and should not be considered
                # if it approaches to 0, means j's socre is the maximal one, it should be scheduled first.
                pus = self.per_user_scores(self.scheduled_rl)
                if not job.user_id in pus:
                    normalized_user_info = 1
                elif max(pus.values()) == 0:
                    normalized_user_info = 1
                else:
                    normalized_user_info = 1 - (float(pus[job.user_id]) / float(max(pus.values())))
                ...
                self.pairs.append([job, normalized_wait_time, normalized_run_time, normalized_request_nodes,
                                   ...
                                   normalized_executable_id, normalized_user_info, can_schedule_now])

            elif self.skip and not add_skip:  # the next job is skip
                add_skip = True
                if self.pivot_job:
                    self.pairs.append([None, 1, 1, 1, 1, 1, 1, 1, 1, 1])
                else:
                    self.pairs.append([None, 1, 1, 1, 1, 1, 1, 1, 1, 0])
            else:
                self.pairs.append([None, 0, 1, 1, 1, 1, 1, 1, 1, 0])
    ...
    def schedule(self, job_for_scheduling):
        ...
        self.running_jobs.append(job_for_scheduling)
        score = self.job_score(job_for_scheduling)  # calculated reward
        self.scheduled_rl[job_for_scheduling] = score
    ...
    def per_user_scores(self, scheduled_logs):
        #print("scheduled logs:", scheduled_logs)
        per_user_average = {}
        per_user_job_count = {}
        for job, score in scheduled_logs.items():
            #print("job:", job.user_id)
            if not job.user_id in per_user_average:
                per_user_average[job.user_id] = score
                per_user_job_count[job.user_id] = 1
            else:
                per_user_average[job.user_id] += score
                per_user_job_count[job.user_id] += 1
                
        for user_id, score in per_user_average.items():
            per_user_average[user_id] = float(score) / float(per_user_job_count[user_id])
        #print("average:", per_user_average)
        return per_user_average
    
    def score_acorss_users(self, lst_of_values):
        if self.user_score_type == 0: # average of all
            return mean(lst_of_values)
        if self.user_score_type == 1: # maximal of all
            return max(lst_of_values)
        if self.user_score_type == 2: # minimal of all
            return min(lst_of_values)
    ...
    def step(self, a):
        ...
        if not done:
            ...
            #print("one traj done:", self.per_user_scores(self.scheduled_rl).values())
            rl_total = self.score_acorss_users(self.per_user_scores(self.scheduled_rl).values())
...
if __name__ == '__main__':
    ...
    env = HPCEnvFair(batch_job_slice=100, build_sjf=True)
    env.seed(0)
