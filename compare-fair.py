...
def run_policy(env, get_probs, get_out, nums, iters, score_type):
    ...
    for iter_num in range(0, iters):
        ...
        sjf_r.append(env.score_acorss_users(env.per_user_scores(env.schedule_curr_sequence_reset(env.sjf_score)).values()))
    ...
    #all_data.append(fcfs_r)
    print("Results:", mean(fcfs_r), mean(wfp_r), mean(uni_r), mean(sjf_r), mean(f1_r), mean(rl_r))
...
if __name__ == '__main__':
    ...
    env = HPCEnvFair(shuffle=args.shuffle, backfil=args.backfil, skip=args.skip, job_score_type=args.job_score_type, 
                user_score_type=args.user_score_type, batch_job_slice=args.batch_job_slice, build_sjf=False)
